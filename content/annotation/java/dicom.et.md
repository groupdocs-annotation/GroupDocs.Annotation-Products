...
############################# Head ############################
head_title: "Java DICOM Annotation API märkused C#-s"
head_description: "Java API populaarsete annotatsioonitüüpide DICOM, piltide, jooniste ja dokumendifailivormingute loomiseks ja märkuste tegemiseks."

############################# Header ############################
title: "Märkige Java faili DICOM"
description: ""
...
about:
    ...
    content: |
        ...
        See teek võimaldab teil töötada mitte ainult DICOM dokumendiga, vaid ka palju muud tüüpi dokumentidega, nagu Word, Excel, PowerPoint, Outlooki meilid, Visio, Adobe, OpenDocument, OpenOffice, Photoshop, AutoCad ja paljud teised.
...
steps_Remove:
    enable: true
    title_left: "Toimingud Java-failist DICOM märkuste eemaldamiseks"
    content_left: |
        [GroupDocs.Annotation](/annotation/java/) muudab Java-arendajate jaoks lihtsamaks annotatsiooni üksikasjade eemaldamise mis tahes Java-põhises rakenduses olevast failist DICOM, rakendades mõnda lihtsat sammu.
        *   Looge vastuseobjektid koos kommentaari ja kuupäevaga.
...
steps_Edit:
    enable: true
    title_left: "Toimingud faili DICOM märkuste muutmiseks Javas"
    content_left: |
        [GroupDocs.Annotation](/annotation/java/) muudab Java-arendajate jaoks lihtsamaks erinevate annotatsioonide atribuutide värskendamise DICOM-failidest mis tahes Java-põhises rakenduses, rakendades mõnda lihtsat sammu.
...
steps_Extract:
    enable: true
    title_left: "Toimingud Java-vormingus failist DICOM märkuste ekstraheerimiseks"
    content_left: |
        [GroupDocs.Annotation](/annotation/java/) muudab Java-arendajate jaoks lihtsaks dokumentidele märkuste lisamise ja annotatsiooniteabe ekstraktimise mis tahes Java-põhises rakenduses olevatest failidest DICOM, rakendades mõne lihtsa sammu.
        *   Looge vastuseobjektid koos kommentaari ja kuupäevaga.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dicom"
          title: "Teave failivormingu DICOM kohta"
          content: |
            DICOM on meditsiinilise digitaalse pildistamise ja kommunikatsiooni akronüüm ning see puudutab meditsiiniinformaatika valdkonda. DICOM on failivormingu määratluse ja võrgusideprotokolli kombinatsioon. DICOM kasutab laiendit .DCM. .DCM on saadaval kahes erinevas vormingus, st vormingus 1.x ja vormingus 2.x. DCM Format 1.x on lisaks saadaval kahes tavalises ja laiendatud versioonis. DICOM-i kasutatakse erinevate tarnijate meditsiiniliste pilditöötlusseadmete (nt printerite, serverite, skannerite jne) integreerimiseks ning see sisaldab ka iga patsiendi identifitseerimisandmeid ainulaadsuse huvides. DICOM-faile saab jagada kahe osapoole vahel, kui nad on võimelised vastu võtma pildiandmeid DICOM-vormingus. DICOM-i suhtlusosa on rakenduskihi protokoll ja kasutab üksuste vahel suhtlemiseks TCP/IP-d. DICOMi veebiteenuste jaoks kasutatakse HTTP- ja HTTPS-protokolle. Veebiteenuste toetatud versioonid on 1.0, 1.1, 2 või uuemad.
