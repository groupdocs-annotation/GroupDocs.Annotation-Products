...
############################# Head ############################
head_title: "Net DICOM Ek Açıklama API'si C#'ta Ek Açıklama"
head_description: "DICOM, resimler, çizimler ve belge dosyası biçimlerinden popüler ek açıklama türleri oluşturmak ve Ek Açıklama eklemek için Net API."

############################# Header ############################
title: "Net'ten DICOM not ekleyin"
description: ""
...
about:
    ...
    content: |
        ...
        Bu kitaplık, yalnızca DICOM belgesiyle değil, Word, Excel, PowerPoint, Outlook e-postaları, Visio, Adobe, OpenDocument, OpenOffice, Photoshop, AutoCad ve diğerleri gibi diğer birçok belge türüyle çalışmanıza olanak tanır.
...
steps_Remove:
    enable: true
    title_left: "Net'teki DICOM Ek Açıklamalarını Kaldırma Adımları"
    content_left: |
        [GroupDocs.Annotation](/annotation/net/) Net geliştiricilerinin, birkaç basit adımı uygulayarak herhangi bir Net tabanlı uygulama içindeki DICOM dosyalarından ek açıklama ayrıntılarını kaldırmasını kolaylaştırır.
        *   Yorum ve tarih içeren Yanıt nesneleri oluşturun.
...
steps_Edit:
    enable: true
    title_left: "Net'te DICOM Ek Açıklamalarını Düzenleme Adımları"
    content_left: |
        [GroupDocs.Annotation](/annotation/net/) Net geliştiricilerinin, birkaç kolay adımı uygulayarak herhangi bir Net tabanlı uygulama içindeki DICOM dosyalarından çeşitli ek açıklama özelliklerini güncellemelerini kolaylaştırır.
...
steps_Extract:
    enable: true
    title_left: "Net'teki DICOM Dosyasından Ek Açıklamaları Çıkarma Adımları"
    content_left: |
        [GroupDocs.Annotation](/annotation/net/) Net geliştiricilerinin belgelere açıklama eklemesini ve birkaç kolay adımı uygulayarak herhangi bir Net tabanlı uygulama içindeki DICOM dosyalarından açıklama bilgilerini çıkarmasını kolaylaştırır.
        *   Yorum ve tarih içeren Yanıt nesneleri oluşturun.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dicom"
          title: "DICOM Dosya Biçimi Hakkında"
          content: |
            DICOM, Tıpta Dijital Görüntüleme ve İletişim'in kısaltmasıdır ve Tıp Bilişimi alanıyla ilgilidir. DICOM, dosya formatı tanımı ile bir ağ iletişim protokolünün birleşimidir. DICOM, .DCM uzantısını kullanır. .DCM iki farklı formatta mevcuttur, yani format 1.x ve format 2.x. DCM Format 1.x'in ayrıca normal ve genişletilmiş iki sürümü mevcuttur. DICOM, çeşitli satıcılardan yazıcılar, sunucular, tarayıcılar vb. tıbbi görüntüleme cihazlarının entegrasyonu için kullanılır ve ayrıca benzersiz olması için her hastanın kimlik verilerini içerir. DICOM dosyaları, DICOM formatında görüntü verisi alabiliyorlarsa iki taraf arasında paylaşılabilir. DICOM'un iletişim kısmı, uygulama katmanı protokolüdür ve varlıklar arasında iletişim kurmak için TCP/IP'yi kullanır. DICOM'un web hizmetleri için HTTP ve HTTPS protokolleri kullanılır. Web servislerinin desteklediği sürümler 1.0, 1.1, 2 veya üstüdür.
