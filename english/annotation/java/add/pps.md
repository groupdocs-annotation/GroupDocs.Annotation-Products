...
steps:
    ...
    content_left: |
        [GroupDocs.Annotation](https://products.groupdocs.com/annotation/java) makes it easy for Java developers to add various annotation types to PPS files within any Java-based application by implementing a few easy steps.
    ...
    code: |
        ```java
        String outputPath = Constants.getOutputFilePath("AddTextStrikeoutAnnotation", FilenameUtils.getExtension(input.pps));
                    final Annotator annotator = new Annotator(input.pps);
                    try {
