...
steps:
    ...
    code: |
        ```java
        String outputPath = Constants.getOutputFilePath("AddTextStrikeoutAnnotation", FilenameUtils.getExtension(input.ppt));
                    final Annotator annotator = new Annotator(input.ppt);
                    try {
