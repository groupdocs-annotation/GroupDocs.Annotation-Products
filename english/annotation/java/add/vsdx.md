...
############################# Head ############################
head_title: "Add Annotations to VSDX in Java Applications"
head_description: "Java API to create and add popular annotation types to VSDX, images, drawings and document file formats."

############################# Header ############################
title: "Annotate VSDX Files in Java"
description: "Add annotations to VSDX, Microsoft Office documents, images, HTML, drawings and other file formats in any type of Java application."
...
steps:
    ...
    content_left: |
        [GroupDocs.Annotation](https://products.groupdocs.com/annotation/java) makes it easy for Java developers to add various annotation types to VSDX files within any Java-based application by implementing a few easy steps.
    ...
    code: |
        ```java
        String outputPath = Constants.getOutputFilePath("AddTextStrikeoutAnnotation", FilenameUtils.getExtension(input.vsdx));
                    final Annotator annotator = new Annotator(input.vsdx);
                    try {
